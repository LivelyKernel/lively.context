...
if (env.isCommonJS) module.require('../index');
...
describe('interpretation', function() {
  ...
  function interpret(node, optMapping) {
    var interpreter = new ast.AcornInterpreter.Interpreter();
  ...
  it('handles this in function calls', function() {
    var node = parse('function foo() { return this; } foo();');
    expect(interpret(node)).to.equal(Global);
  });
  ...
  it('handles simple try without catch but with finally (with error)', function() {
    ...
    expect(lang.fun.curry(interpret, node)).to.throw({ a: 1 });
  });
  ...
  it('handles try and catch with variable change in finally', function() {
    ...
    expect(lang.fun.curry(interpret, node, mapping)).to.throw(4);
  ...
  it('can instantiate class', function() {
    ...
    var klass = lang['class'].create('lively.' + className, { a: 1 }),
        src = lang.string.format('var obj = new lively.%s(); obj.a;', className),
        node = parse(src);
    expect(interpret(node, { lively: lively })).to.equal(1);
    expect(lively.lang.classHelper.isClass(Global.lively[className])).to.be.true;
  });

  it('can instantiate class with constructor arguments', function() {
    var className = 'Dummy_test27ArgumentsOfConstructorAreUsed';
    after(function() { delete Global.lively[className]; });
    lang['class'].create('lively.' + className, { initialize: function(n) { this.n = n; } });
    var src = lang.string.format('var obj = new lively.%s(1); obj.n;', className),
        node = parse(src);
    expect(interpret(node, { lively: lively })).to.equal(1);
  });
  ...
  it('throws an error if arguments is used outside of a function', function() {
    ...
    expect(lang.fun.curry(interpret, node)).to.throw(Error);
  });
  ...
  it('returns real function for a function', function() {
    ...
    expect(lang.obj.isFunction(result)).to.be.true;
  });
  ...
  it('can read undefined variable from global scope', function() {
    ...
    expect(lang.fun.curry(interpret, node, Global)).to.throw(Error); // undefined variable read threw an error
  });
  ...
  it('throws UnwindException for debugger statements', function() {
    ...
    expect(lang.fun.curry(interpret, node)).to.throw(/UNWIND.*Debugger/);
  });
  ...
  it('errors when accessing property of an undefined variable', function() {
    var src = 'var foo; foo.bar',
        node = parse(src);
    expect(lang.fun.curry(interpret, node)).to.throw(/undefined/); // FIXME: error messages are implementation dependent (use to be: /TypeError.*property.*bar.*undefined/)
  });
